...
namespace anatomy::hand {
    ...
    Status Finger::flex(Flexion flexion) {
        return flex_joint_->move_to(1.0f - flexion.percentage, flexion.time_to_execute_ms);
    }

    Status Finger::extend(Extension extension) {
        return flex_joint_->move_to(extension.percentage, extension.time_to_execute_ms);
    }
