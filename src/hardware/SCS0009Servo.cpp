...
namespace hardware {
    ...
    void SCS0009Servo::move(int target_step, float target_speed_steps_per_sec) {
        ...
        set_speed(id_, static_cast<uint16_t>(target_speed_steps_per_sec));
        move_steps(id_, static_cast<uint16_t>(target_step));
    }
    ...
    const int SCS0009Servo::read_current_step() {
        return read_present_position(id_);
    }

    const float SCS0009Servo::read_load_percent() {
        int16_t load = read_present_load(id_);
    ...
    void SCS0009Servo::freeze() {
        frozen_.store(true);
        set_torque_enable(id_, false);
    }

    void SCS0009Servo::unfreeze() {
        frozen_.store(false);
        set_torque_enable(id_, true);
    }
